  ...
  const handleDownloadInvoice = async () => {
    ...
    const invoiceHTML = `
      <div class="container" style="padding: 2.5rem; font-family: 'Inter', sans-serif; color: #333;">
        ...
        <h2 style="text-align: center; letter-spacing: 0.2rem; margin-bottom: 2rem;">INVOICE</h2>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; margin-bottom: 2rem;">
          <div>
            <h3 style="font-size: 0.8rem; text-transform: uppercase; color: #999; margin-bottom: 0.5rem;">Bill To</h3>
            <p style="margin: 0.2rem 0;"><strong>${escapeHtml(safeOrder.customerName)}</strong></p>
            <p style="margin: 0.2rem 0;">${escapeHtml(safeOrder.address)}</p>
            <p style="margin: 0.2rem 0;">${escapeHtml(safeOrder.phone)}</p>
          </div>
          <div style="text-align: right;">
            <p style="margin: 0.2rem 0;"><strong>Invoice No:</strong> #${escapeHtml(safeOrder.id)}</p>
            <p style="margin: 0.2rem 0;"><strong>Date:</strong> ${escapeHtml(safeOrder.orderDate)}</p>
            <p style="margin: 0.2rem 0;"><strong>Status:</strong> ${escapeHtml(safeOrder.status)}</p>
          </div>
        </div>
        ...
        <div style="display: flex; justify-content: flex-end;">
          <div style="width: 250px;">
            <div style="display: flex; justify-content: space-between; padding: 0.5rem 0;"><span>Subtotal</span><span>Rs. ${totalPrice.toFixed(2)}</span></div>
            ...
            <div style="display: flex; justify-content: space-between; padding: 0.5rem 0;"><span>Delivery</span><span>Rs. ${deliveryCharge.toFixed(2)}</span></div>
            <div style="display: flex; justify-content: space-between; padding: 0.5rem 0; border-top: 2px solid #eee; margin-top: 0.5rem; font-weight: 700; color: #FF2E36; font-size: 1.1rem;">
              <span>Total</span><span>Rs. ${codAmount.toFixed(2)}</span>
            </div>
          </div>
        </div>
    ...
    try {
      if (Capacitor.isNativePlatform()) {
        ...
        const path = `Invoice_${safeOrder.id}.pdf`
